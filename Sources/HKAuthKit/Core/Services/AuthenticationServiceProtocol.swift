import Foundation
import FirebaseAuth

public protocol AuthenticationServiceProtocol {
    var currentUser: FirebaseAuth.User? { get }
    var isAuthenticated: Bool { get }
    
    // Authentication Methods
    func signIn(email: String, password: String) async throws -> FirebaseAuth.User
    func signUp(email: String, password: String, fullName: String) async throws -> FirebaseAuth.User
    func signOut() throws
    
    // Password Management
    func resetPassword(email: String) async throws
    func updatePassword(currentPassword: String, newPassword: String) async throws
    
    // Social Authentication
    func signInWithGoogle() async throws -> FirebaseAuth.User
    func signInWithApple() async throws -> FirebaseAuth.User
    
    // User Management
    func updateProfile(fullName: String?, profileImageUrl: String?) async throws -> FirebaseAuth.User
    func deleteAccount() async throws
    
    // Biometric Authentication
    func enableBiometricAuth() async throws
    func disableBiometricAuth() throws
    func authenticateWithBiometrics() async throws -> FirebaseAuth.User
}
